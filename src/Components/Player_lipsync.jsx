...
export default function Player_lipsync({ isSpeaking, isIdle, audio, lipsync }) {
  const group = useRef();
  const { scene } = useGLTF('/src/assets/LTI Avatar.glb');
  const {animations}=useGLTF('/src/assets/animations.glb');
  ...
  return (
    <group ref={group} dispose={null} position={[2, -4, 2]} scale={[4, 4, 4]}>
...
useGLTF.preload('/LTI Avatar.glb');